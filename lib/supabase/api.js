...
// New paginated and sorted listings function
export async function getListingsPaginated({
  ...
  // Build the query - fetch all data first to sort properly
  let query = supabase
    .from('listings')
    .select('*', { count: 'exact' });
  ...
  if (error) {
    console.error('Supabase error fetching paginated listings:', error.message);
    throw new Error('Could not fetch listings.');
  }

  // Sort the data manually in JavaScript for precise control
  const sortedData = [...(data || [])].sort((a, b) => {
    // Primary sort: available items first (true before false)
    if (a.is_available !== b.is_available) {
      return b.is_available - a.is_available; // true (1) comes before false (0)
    }
    
    // Secondary sort: by the user's chosen field
...
export async function bulkDeleteListings(supabase, ids) {
  if (!ids || ids.length === 0) {
    throw new Error('No listing IDs provided for deletion.');
  }
  ...
  if (error) {
    console.error('Supabase error bulk deleting listings:', error.message);
    throw error;
  }
...
export async function bulkUpdateListings(supabase, ids, updateData) {
  if (!ids || ids.length === 0) {
    throw new Error('No listing IDs provided for update.');
  }
  ...
  if (error) {
    console.error('Supabase error bulk updating listings:', error.message);
    throw error;
  }
