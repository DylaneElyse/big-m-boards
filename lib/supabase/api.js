...
// 1. UPDATED SCHEMA: Added the 'slug' and 'price' fields.
...
export async function getListingById(supabase, id) {
  ...
  if (!data || data.length === 0) {
    // Don't log as error since this is expected when listing doesn't exist
  ...
  // Ensure image_urls is properly parsed as a JavaScript array
  if (listing && listing.image_urls) {
    // Force conversion to a proper JavaScript array
    try {
      // First try to stringify and parse to ensure it's a proper array
      ...
      // Fallback: try to extract URLs manually if it's an object with numeric keys
...
export async function getDashboardStats() {
  ...
  try {
    ...
    // Get average price (for listings with prices)
