...
export default function PublicListingsClient({ initialData }) {
  ...
  const initialParamsRef = useRef({
    ...
    sort: 'created_at',
    order: 'desc',
    filter: 'all' // Default to all listings for public
  });
      ...
      {!loading && (!listings || listings.length === 0) ? (
        <div className="text-center py-16">
          ...
          <p className="mt-1 text-sm text-gray-500">
            {currentFilter !== 'available' || currentSort !== 'created_at' 
        ...
        <>
          <div className={`grid grid-cols-2 gap-y-6 gap-x-3 sm:gap-y-10 sm:gap-x-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 ${loading ? 'opacity-50' : ''}`}>
            {listings.map((listing) => {
              ...
              return (
                <Link 
                  href={`/listings/${listing.slug}?returnTo=${encodeURIComponent(returnUrl)}`}
                  key={listing.id} 
